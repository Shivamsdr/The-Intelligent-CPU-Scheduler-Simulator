...
def suggest_algorithm(processes):
    ...
    '''third check'''
    if btme1 - btme2 > 2 * burst_time_stddev:  # Small processes arrived later
        return "srtf"
    
    '''fourth check - consider rr with all others respecting the avg burst'''
    if arrival_time_stddev < 2: #checks for low preemptivity
